...
export default function Charts3D({ rows = [], classifiedRows = null }) {
  ...
  const byWallet = summarizeByWallet(rows);
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
      ...
      {byWallet.length > 1 && (
        <ChartPanel title="Gain / loss by wallet" data={byWallet} colorMode="signed" />
      )}
    </div>
  );
}

function summarizeByWallet(rows) {
  const byWallet = new Map();
  for (const r of rows) {
    if (r.gain == null) continue;
    const label = r.walletLabel || 'Unlabeled';
    byWallet.set(label, (byWallet.get(label) || 0) + r.gain);
  }
  return [...byWallet.entries()].map(([label, gain]) => ({ label, value: gain }));
}
